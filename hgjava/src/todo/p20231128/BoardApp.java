...
public class BoardApp {
	//static BoardExe exe = new BoardExe(); //페이징기능 사용 main메소드 밖에 만들어서 이것도 밖에 빼서 선언 (이건 한번만 적으면 계속 사용가능)
	   //--- BoardExe 정적 static으로 바꿔서 위에 new로 인스턴스 만들 필요없어지고 주석처리함/ 이것도 exe기능 실행 그러 ㅁ 안되기때문에 BoardExe.으로 다 바꿔줌
	...
	private void boardAdd(){
		...
		Board board = new Board(BoardExe.getSequence(), head, id, contents, dateTime); //사용할때마다 계속 생성되어야됨(그래서while구무 ㄴ 안에있음)
		// -> 값 boards 배열에 담기
